...
function AddAgent() {
    ...
    try {
      ...
      if (token) {
        const response = await axios.get(
          `${Backend_EndPoint}api/v1/user/waiting-users`,
          {
            headers: {
  ...
  const FetchAllUser = async () => {
    try {
      ...
      if (token) {
        ...
        if (response.status === 200) {
          const agentsOnly = response.data.filter(
            (agent: RequestDetails) => agent.role === "Agent",
          );
  ...
  const displayedRequest = requestList.slice(
    ...
    try {
      if (token) {
        ...
        const response = await axios.post(
          `${Backend_EndPoint}api/v1/user/approve-user`,
          data,
          {
            headers: {
              Authorization: `Bearer ${token}`,
  ...
  const RejectUser = async (id: String) => {
    try {
      if (token) {
        const data = JSON.stringify({ id: id });
        const response = await axios.post(
          `${Backend_EndPoint}api/v1/user/reject-user`,
          data,
          {
            headers: {
              Authorization: `Bearer ${token}`,
